 ...
 */
export function validAirQualityBox(params) {
  ...
  if (north - south > AQ_MAX_BOX_DEGREES || east - west > AQ_MAX_BOX_DEGREES)
    return null;
...
export function resolveGridSide(value) {
  if (value === null || value === undefined || String(value).trim() === '')
    return AQ_GRID_SIDE_DEFAULT;
  const side = Number(value);
  if (!Number.isFinite(side)) return AQ_GRID_SIDE_DEFAULT;
  return Math.min(
    AQ_GRID_SIDE_MAX,
    Math.max(AQ_GRID_SIDE_MIN, Math.round(side)),
  );
}
 ...
 */
export function quantizeAirQualityBox(box) {
...
export function airQualityCacheKey(box, side) {
 ...
 * middle of what it colours. Sampling corners would bias every cell toward its
 * south-west neighbour.
 * @param {{south:number, west:number, north:number, east:number}} box Viewport.
 * @param {number} side Cells per axis.
 * @returns {Array<{latitude:number, longitude:number, south:number, west:number, north:number, east:number}>} Cells.
 */
...
export function airQualityFailureReason(error) {
  const reason = error?.airQualityReason;
